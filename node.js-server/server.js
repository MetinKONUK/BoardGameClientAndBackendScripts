...
const DeleteBoard = (board) => {
	matches = matches.filter((brd) => brd !== board);
}
...
const HandleMessage = (ws, data) => {
	...
	if (DataFromClient.Type == "opponent_username") // MAKE CLIENTS OPPONENT
	{
		...
		if (Opponent == null) {
			...
			DataToClient.Type = "opponent-does-not-exist-error";
			SendDataToClients(DataToClient, [ws]);
			return;
		}
...
const DisconnectClient = (ws) => {
	...
	let brd = FindClientsBoard(ws.Username);
	if (brd != null) {
		DataToClient.Type = "opponent-disconnected";
		if (ws != brd.player1) {
			SendDataToClients(DataToClient, [brd.player1]);
		} else {
			SendDataToClients(DataToClient, [brd.player2]);
		}
	}
}
